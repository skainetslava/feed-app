...
const Track: React.FC<ITrackComponentProps> = ({
  ...
  return (
    <div className={cls("track", { "track--active": isActive }, { "track--playing": isPlaying })}>
      {isPlaying && isActive && (
        <IconVolume className="track_on" onClick={() => onPauseAudio && onPauseAudio()} />
      )}
      {!isActive && (
        <IconPlay className={cls("track_play")} onClick={() => onPlayAudio && onPlayAudio(track)} />
      )}
      {!isPlaying && isActive && (
        <IconPlay
      ...
      <IconPause className={cls("track_pause")} onClick={() => onPauseAudio && onPauseAudio()} />

      {track.coverSmallTrack && (
        <img className="track_cover" src={track.coverSmallTrack} alt={track.title} />
      )}

      <div className="track_info">
        <p className="track_title">{track.title}</p>
        <Link to={`/artist/${track.artistId}`} aria-label="Go to artist" className="track_artist">
          {track.artist}
        </Link>
      </div>
